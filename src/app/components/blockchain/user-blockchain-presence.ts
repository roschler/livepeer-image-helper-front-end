...
import {
    EthereumProvider,
    HEX_UNINITIALIZED_VALUE,
    isHexUninitializedValue,
...
import {createWalletClient, custom, Account, http, Hex, isHex, defineChain, toHex} from 'viem';
...
} from "@story-protocol/core-sdk/dist/declarations/src/types/resources/ipAsset";
import {SimpleWalletClient} from "@story-protocol/core-sdk/dist/declarations/src/abi/generated";
import {LicenseAssistantNuevoResponse, MintNftImageDetails} from "@/app/reader/types";
import {CreateNFTCollectionRequest} from "@story-protocol/core-sdk/dist/declarations/src/types/resources/nftClient";
import {CurrencyAddress, RPCProviderUrl} from "@/story-protocol/utils";
import {LicenseTerms} from "@story-protocol/core-sdk/dist/declarations/src/types/resources/license";
 ...
 * @returns The JSON string with BigInt values converted to strings, or an empty string if serialization fails.
 */
/* eslint-disable */
function serializeWithBigInt(obj: Record<string, unknown>): string {
    try {
        ...
        // Return an empty string if JSON.stringify returns undefined
        return '';
    } catch (error) {
        // In case of unexpected errors (like circular references), return an empty string
        return '';
    }
}
/* eslint-enable */
 ...
 * @returns The object with BigInt values restored, or `null` if deserialization fails.
 */
/* eslint-disable */

function deserializeWithBigInt(jsonStr: string): Record<string, unknown> | null {
    try {
        return JSON.parse(jsonStr, (_key, value) => {
            ...
            return value;
        }) as Record<string, unknown>;  // Safely cast the result to a Record
    } catch (error) {
        // Return `null` if the string is not valid JSON
        return null;
    }
}
...
export class UserBlockchainPresence {
     ...
     * @param licenseResponseObj - The license terms specified by
     *  the user for the NFT along with the numeric details
     *  required for the selected license.
     *
     * @returns - Returns the response object that contains
     *  the mint and register results.
     */
    public async mintAndRegisterNft(
        imageDetailsForNftMinting: MintNftImageDetails,
        licenseResponseObj: LicenseAssistantNuevoResponse): Promise<MintNftRequestAndResponse> {
        ...
        function ethToGwei(eth: number): number {
            const gweiPerEth = 1e9; // 1 ETH = 1,000,000,000 Gwei
            const gwei = eth * gweiPerEth;
            return Math.trunc(gwei); // Truncate to the nearest integer
        }

        // Need to convert Eth to Gwei.
        const numGweiMintingFee =
            ethToGwei(licenseResponseObj.defaultMintingFee);

        if (licenseResponseObj.best_license_guess === String(EnumStoryProtocolLicenses.NON_COMMERCIAL_SOCIAL_REMIXING)) {
            mintAndRegisterRequestObj.pilType = PIL_TYPE.NON_COMMERCIAL_REMIX;
        } else if (licenseResponseObj.best_license_guess === String(EnumStoryProtocolLicenses.COMMERCIAL_USE)) {
            mintAndRegisterRequestObj.pilType = PIL_TYPE.COMMERCIAL_USE;

            // Need minting fee too.
            mintAndRegisterRequestObj.mintingFee = numGweiMintingFee;

            // And currency.
            mintAndRegisterRequestObj.currency = CurrencyAddress;
        } else if (licenseResponseObj.best_license_guess === String(EnumStoryProtocolLicenses.COMMERCIAL_REMIX)) {
            mintAndRegisterRequestObj.pilType = PIL_TYPE.COMMERCIAL_REMIX;

            // Need minting fee too.
            mintAndRegisterRequestObj.mintingFee = numGweiMintingFee;

            // Need the rev share percentage too.
            mintAndRegisterRequestObj.commercialRevShare = licenseResponseObj.commercialRevShare;

            // And currency.
            mintAndRegisterRequestObj.currency = CurrencyAddress;
        } else {
            throw new Error(`Don't know how to handle selected license type of: ${licenseResponseObj.best_license_guess}`);
        }

        // Mint and register it with the license terms selected.
     ...
     */
    public toJsonString() {
        return serializeWithBigInt(this as Record<string, unknown>)
    }
    ...
    static fromJsonString(jsonStr: string): UserBlockchainPresence {
        ...
        // for BigInt fields.
        const rawJson = deserializeWithBigInt(jsonStr);

        if (rawJson === null) {
            throw new Error(`Failed to deserialize JSON string.`);
        }
